...
export async function executeDiscordSync(actor: SyncActor, mode: string = 'delta'): Promise<SyncResult> {
  ...
  try {
    ...
    try {
      ...
      console.error('failed to update sync status on discord sync error:', dbErr);
    }
