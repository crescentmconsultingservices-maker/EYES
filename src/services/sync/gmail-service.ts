...
export async function executeGmailSync(actor: SyncActor, mode: string = 'delta'): Promise<SyncResult> {
  try {
    ...
    if (activeAudit) {
      return { status: 423, error:  'System Busy: Reputation Audit in progress.', detail:  'Ingestion is paused to ensure data snapshot integrity for your current audit.' 
       }; // 423 Locked
    }
    ...
    if (!accessToken) {
      return { status: 401, error:  'Gmail session expired and refresh failed.'  };
    }
    ...
    return { status: 500, error:  'Unable to sync Gmail data.', 
      detail
     };
  }
}
