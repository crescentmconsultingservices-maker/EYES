...
export async function executeGoogleSheetsSync(actor: SyncActor, mode: string = 'delta') {
  ...
  try {
    accessToken = await getValidGoogleToken(supabase, userId, 'google-sheets');
  } catch (err) {
    return { status: 401, error: 'Google Sheets authentication failed.', detail: String(err) };
  }

  if (!accessToken) {
    return { status: 401, error: 'Google Sheets is not connected yet.' };
  }
  ...
  const mimeTypeQuery = encodeURIComponent(`mimeType='application/vnd.google-apps.spreadsheet'`);
  ...
  if (!response.ok) {
    return { status: response.status, error: 'Failed to fetch Google Sheets' };
  }
  ...
  const rawEvents = files.map((file: any) => ({
    ...
    content: `Google Sheet URL: ${file.webViewLink}`,
  ...
  if (hasMore && isBackfill) {
    try {
      ...
      console.warn('[Google Sheets Sync] Could not schedule next QStash sync chunk:', qErr);
    }
  }
