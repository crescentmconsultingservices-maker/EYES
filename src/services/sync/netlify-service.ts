import { upsertRawEventsSafely, upsertSyncStatusSafely } from '@/utils/supabase/upsert';
import { getValidNetlifyToken } from '@/services/auth/oauth';
import { type SyncActor } from '@/utils/sync/actor';

export async function executeNetlifySync(actor: SyncActor, mode: string = 'delta') {
  const { supabase, userId, userEmail, userName } = actor;

  const { data: profile } = await supabase
    .from('user_profiles')
    .select('account_type, organization_id')
    .eq('user_id', userId)
    .maybeSingle();

  const isOrg = profile?.account_type === 'organization' && profile?.organization_id;
  const orgId = isOrg ? profile.organization_id : null;

  const { data: activeAudit } = await supabase
    .from('reputation_audits')
    .select('id, status')
    .eq('user_id', userId)
    .in('status', ['pending', 'analysis', 'generating'])
    .maybeSingle();

  if (activeAudit) {
    return { status: 423, error: 'System Busy: Reputation Audit in progress.', detail: 'Ingestion is paused.' };
  }

  const { data: currentStatus } = await supabase
    .from('sync_status')
    .select('cursor, total_items, last_sync_at')
    .eq('user_id', userId)
    .eq('platform', 'netlify')
    .maybeSingle();

  let accessToken: string | null = null;
  try {
    accessToken = await getValidNetlifyToken(supabase, userId);
  } catch (err) {
    return { status: 401, error: 'Netlify authentication failed.', detail: String(err) };
  }

  if (!accessToken) {
    return { status: 401, error: 'Netlify is not connected yet.' };
  }

  const isBackfill = mode === 'backfill';
  const perPage = 50;
  let page = isBackfill ? parseInt(currentStatus?.cursor || '1') : 1;

  await upsertSyncStatusSafely(supabase, {
    user_id: userId,
    platform: 'netlify',
    status: 'syncing',
    last_sync_at: new Date().toISOString(),
  });

  const response = await fetch(`https://api.netlify.com/api/v1/sites?per_page=${perPage}&page=${page}`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      Accept: 'application/json',
    },
  });

  if (!response.ok) {
    return { status: response.status, error: 'Failed to fetch Netlify sites' };
  }

  const sites = await response.json() || [];
  const hasMore = sites.length === perPage;

  const rawEvents = sites.map((site: any) => ({
    user_id: userId,
    platform: 'netlify',
    platform_id: String(site.site_id),
    event_type: 'deployment',
    title: site.name,
    content: `Netlify Site: ${site.url}, Build state: ${site.build_settings?.provider}`,
    author: userEmail || userName || 'Netlify User',
    timestamp: new Date(site.updated_at).toISOString(),
    scope: isOrg ? 'organizational' : 'personal',
    organization_id: orgId,
    metadata: {
      id: site.id,
      url: site.url,
      admin_url: site.admin_url,
    },
    is_flagged: false,
    flag_severity: 'none',
    flag_reason: null,
  }));

  if (rawEvents.length > 0) {
    await upsertRawEventsSafely(supabase, rawEvents);
  }

  const now = new Date().toISOString();
  await Promise.all([
    upsertSyncStatusSafely(supabase, {
      user_id: userId,
      platform: 'netlify',
      status: hasMore ? 'syncing' : 'connected',
      sync_progress: hasMore ? 60 : 100,
      total_items: (currentStatus?.total_items || 0) + rawEvents.length,
      last_sync_at: now,
      next_sync_at: new Date(Date.now() + 1000 * 60 * 30).toISOString(),
      cursor: hasMore ? String(page + 1) : null,
      error_message: null,
    }),
    supabase.from('user_profiles').update({
      memories_indexed: (currentStatus?.total_items || 0) + rawEvents.length,
      updated_at: now,
    }).eq('user_id', userId),
  ]);

  if (hasMore && isBackfill) {
    try {
      const { dispatchNextSyncJob } = await import('@/services/sync/queue-dispatcher');
      await dispatchNextSyncJob({
        userId,
        platform: 'netlify',
        mode: 'backfill',
        cursor: String(page + 1),
        delaySeconds: 3,
      });
    } catch (qErr) {
      console.warn('[Netlify Sync] Could not schedule next QStash sync chunk:', qErr);
    }
  }

  return {
    status: 200,
    data: { ok: true, syncedItems: rawEvents.length, hasMore }
  };
}
