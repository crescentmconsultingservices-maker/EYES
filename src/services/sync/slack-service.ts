...
export async function executeSlackSync(actor: SyncActor, mode: string = 'delta'): Promise<SyncResult> {
  ...
  try {
    ...
    if (hasMoreOverall && mode === 'backfill') {
      try {
        ...
        console.warn('[Slack Sync] Could not schedule next QStash sync chunk:', qErr);
      }
    }
